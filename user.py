...
def reset_simulation():
    ...
    user_profit_var.set("Profit: $0.00")
    user_demand_var.set("Demand: 0 customers")
    user_rank_var.set("Rank: -")
...
def simulate_day():
    ...
    if current_day % 15 == 0:  # Market disruption every 15 days
        ...
        # Don't modify user-controlled airline's price
        for i, airline in enumerate(airlines):
            if not (airline == user_controlled_airline and user_control_enabled):
                prices[i] = prices[i] * shock_magnitude
                
        print(f"Day {current_day}: Market disruption! All AI airline prices adjusted by factor {shock_magnitude:.2f}")
        ...
        if is_running:
            messagebox.showinfo("Market Disruption", 
                               ...
                               f"All AI airline prices adjusted by {(shock_magnitude-1)*100:.1f}%.\n\n"
                               f"This is a good time to adjust your price strategy.")
            
            # Prompt user for price change after disruption
            if user_control_enabled:
                get_user_price_input()

    # Prompt user for price decision every 5 days
    elif current_day % 5 == 0 and user_control_enabled and is_running:
        get_user_price_input()
...
def start_simulation():
    ...
    if not is_running:
        ...
        # If user control is enabled, prompt for initial price before starting
        if user_control_enabled and current_day == 0:
            get_user_price_input()
        simulate_day()
    else:
        is_running = False
...
# Set Price Button for user-controlled airline
set_price_button = tk.Button(user_control_frame, text="Set Your Price", command=set_user_price)
set_price_button.pack(side=tk.RIGHT, padx=10)

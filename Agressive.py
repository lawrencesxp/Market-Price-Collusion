...
# Collusion bonus settings - ENHANCED
collusion_band_width = 0.075  # Narrower band (7.5% around average price)
collusion_reward_bonus = 8000  # Significantly higher bonus for staying in collusion band
# Higher reward when prices are higher than base cost by a good margin
price_premium_threshold = 100  # Bonus scales up when prices > $100
...
def calculate_collusion_bonus(price, all_prices, profits):
    ...
    if is_in_collusion_band(price, avg_price):
        ...
        # NEW: Enhanced collusion rewards when everyone is in the band
        if band_factor > 0.8:  # If 80%+ of airlines are colluding
            band_factor *= 1.5  # Boost the reward by 50%
        
        # Scale bonus by profitability - higher bonus for profitable collusion
        profit_factor = min(1.0, max(0.0, np.mean(profits) / 10000))
        
        # NEW: Price premium bonus - reward higher prices more
        price_premium = 1.0
        if avg_price > price_premium_threshold:
            # Bonus scales with how much above the threshold we are
            premium_ratio = (avg_price - price_premium_threshold) / 50  # $50 above threshold = 100% bonus
            price_premium = 1.0 + min(2.0, premium_ratio)  # Cap at tripling the reward
